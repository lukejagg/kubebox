    ...
    for resource in resources:
        if resource.get('type') == 'azurerm_kubernetes_cluster':
            for instance in resource.get('instances', []):
                ...
                kube_config_raw = attributes.get('kube_config_raw') or attributes.get('kube_admin_config_raw')
                if kube_config_raw:
...
def create_pod_and_service(api_instance, core_v1, user_name, image):
    ...
    # Create the pod
    try:
        api_response = core_v1.create_namespaced_pod(namespace='default', body=pod_manifest)
    ...
    # Create the service
    try:
        api_response = core_v1.create_namespaced_service(namespace='default', body=service_manifest)
        ...
        try:
            ...
            if status.phase == 'Running':
                ...
                if all_ready:
                    ...
                    print(f"Pod '{pod_name}' is ready. Time taken: {elapsed:.2f} seconds.")
                    return elapsed
        except ApiException as e:
...
def main():
    ...
    pod_name, service_name = create_pod_and_service(client.ApiClient(), core_v1, USER_NAME, DOCKER_IMAGE)
